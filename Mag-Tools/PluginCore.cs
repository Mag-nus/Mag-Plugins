...
namespace MagTools
{
    ...
    public sealed class PluginCore : PluginBase, IPluginCore
    {
        ...
        public bool ProcessMTCommand(string mtCommand)
        {
                ...
                (lower.StartsWith("/mt usel ") && lower.Length > 9) || (lower.StartsWith("/mt uselp ") && lower.Length > 10))
            {
                ...
                else
                {
                    ...
                    useMethod = FindIdForName(second, searchInventory, false, searchLandscape, false, partialMatch, objectId);
                }
            ...
            if (CoreManager.Current.Actions.VendorId != 0)
            {
                ...
                if (lower.StartsWith(cmd))
                {
                    ...
                    int itemQty = commands.Length > 1 ? Int32.Parse(commands[1]) : 1;
                ...
                if (lower.StartsWith(cmd))
                {
                    ...
                    if (itemValue == -1)
                    {
                        Debug.WriteToChat("Format: item name|itemVendorValue");
                        return true;
                    }
                ...
                if ((lower.StartsWith("/mt vendor addbuy ") && lower.Length > 18) || (lower.StartsWith("/mt vendor addbuyp ") && lower.Length > 19))
                {
                    ...
                    int objectId = FindIdForName(itemName, false, false, false, true, partialMatch);
        ...
        int FindIdForName(string name, bool searchInInventory, bool searchOpenContainer, bool searchEnvironment, bool searchVendor, bool partialMatch, int idToSkip = 0)
        {
            ...
            if (searchVendor)
            {
                if (vendorList.ContainsKey(name.ToLower()))
                {
